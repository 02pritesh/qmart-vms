...
@section('admin-content')
    ...
    @endif
      <a href="{{url('innvoice-mrn-detail')}}" class="btn-submit ml-3" style="text-decoration: none; padding:4px 14px;"><i class="fa-solid fa-left-long"></i></a>
    <!--<h3 class="text-center" style="color:#000;font-family: Silka-Black;"><b>Request Monthly Sales Report/Statement</b></h3>-->
 <form action="{{ url('edit-innvoice-message') }}" method="POST" enctype="multipart/form-data">
      @csrf
                      ...
                      <input type="hidden" name="existing_vendor_file" value="{{ $messages->vendor_file }}">
                        @if (in_array(pathinfo($messages->vendor_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'pdf','doc','txt','bmp','xlsx','xls','csv','zip']))
                             ...
                             @if (in_array(pathinfo($messages->vendor_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png','bmp']))
                                ...
                                <div id="vendorImagePreview">
                                    <a href="{{ asset('public/assets/upload/innvoices' . $messages->vendor_file) }}" download>
                                        <img src="{{ asset('public/assets/upload/innvoices' . $messages->vendor_file) }}" alt="Uploaded File" style="max-width: 8%; height: auto;">
                                    </a>
                                    <!--<input type="hidden" name="existing_vendor_file" value="{{ $messages->vendor_file }}">-->
                                </div>
                            ...
                            @elseif(in_array(pathinfo($messages->vendor_file, PATHINFO_EXTENSION), ['pdf','docx','doc','txt','xlsx','xls','zip']))
                                ...
                                <div class="form-group">
                                    <a href="{{ asset('public/assets/upload/innvoices' . $messages->vendor_file) }}" class="btn-submit" style="text-decoration:none;" download>Download File</a>
                                </div>
                            @endif
    ...
    <div class="container block mt-4 mb-4">
            ...
            <div class="row">
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" name="existing_admin_file" value="{{ $messages->admin_file }}">
                        @if (in_array(pathinfo($messages->admin_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'pdf','xlsx','xls','zip']))
                            ...
                            @if (in_array(pathinfo($messages->admin_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png']))
                                <div id="adminImagePreview">
                                    <a href="{{ asset('public/assets/upload/innvoices' . $messages->admin_file) }}" download>
                                        <img src="{{ asset('public/assets/upload/innvoices' . $messages->admin_file) }}" alt="Uploaded File" style="max-width: 8%; height: auto;">
                                    </a>
                            ...
                            @elseif(in_array(pathinfo($messages->admin_file, PATHINFO_EXTENSION),['pdf','xlsx','xls','zip']))
                                ...
                                <a href="{{ asset('public/assets/upload/innvoices' . $messages->admin_file) }}" class="btn-submit mt-3" style="text-decoration:none; font-size:12px;" target="_blank" download>View Uploaded File</a>
                            @endif
            ...
            @elseif($messages->status == 'Aligned' && session()->get('adminloginId') == 1)
                 <button type="submit" class="btn-submit btn-report mr-3 mb-3">Submit</button>
                {{-- <button type="reset" class="btn-submit btn-reset btn-danger mb-3">Reset</button> --}}
            @endif
