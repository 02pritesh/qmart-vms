...
@section('user-content')
    ...
    <a href="{{ url('vendor-show-credit-detail') }}" class="btn-submit ml-3"
    ...
    @if ($messages->admin_message)
                    ...
                    @if ($messages->admin_file)
                        @if (in_array(pathinfo($messages->admin_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif']))
                            ...
                            <a href="{{ asset('public/assets/upload/debit/' . $messages->admin_file) }}" download>
                                <img src="{{ asset('public/assets/upload/debit/' . $messages->admin_file) }}"
                        ...
                        @else
                            ...
                            <a href="{{ asset('public/assets/upload/debit/' . $messages->admin_file) }}" class="btn-submit"
                                style="text-decoration:none; font-size:12px;" target="_blank" download>View Uploaded
                                File</a>
                        @endif
                    @else
        ...
        <div class="container block mt-4 mb-4">
            ...
            <form action="{{ url('vendor-debit-reply') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                        ...
                        @if ($messages->vendor_file)
                            @if (in_array(pathinfo($messages->vendor_file, PATHINFO_EXTENSION), ['jpg', 'jpeg', 'png', 'gif']))
                                ...
                                <a href="{{ asset('public/assets/upload/debit/' . $messages->vendor_file) }}" download>
                                    <img src="{{ asset('public/assets/upload/debit/' . $messages->vendor_file) }}"
                                        alt="Uploaded File" style="max-width: 8%; height: auto;">
                                </a>
                            ...
                            @else
                                <div class="form-group">
                                    <a href="{{ asset('public/assets/upload/debit/' . $messages->vendor_file) }}"
            ...
            <div class="row">
                ...
                <div class="form-group">
                    ...
                    @if ($messages->admin_file)
                        ...
                        @else
                            <!-- Display file if it is not an image -->
                            <a href="{{ asset('public/assets/upload/debit/' . $messages->admin_file) }}"
